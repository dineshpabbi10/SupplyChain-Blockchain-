...
contract(SupplyChain,([deployer, manufacturer1, manufacturer2])=>{
    let business;
    before(async () =>{
    ...
    describe('Creating orders for manufactuers, changing its status and completing the orders', async () => {
        ...
        it ('Giving order should be successful if all correct', async ()=>{
            //SUCCESSFUL
            result = await business.giveOrderToManufacturer('fur', 1, manufacturer1, {from: deployer, value: web3.utils.toWei('1', 'Ether')})
        ...
        /*it ('Order should be rejected if component name is not from the set of predefined components', async ()=>{
            ...
            await business.giveOrderToManufacturer('silk', 5, manufacturer1, {from: deployer, value: web3.utils.toWei('5', 'Ether')}).should.be.rejected;
...
/*
        ...
        it ('Order status to be updated as inProgress only by the assigned manufacturer', async ()=>{
          //  orderNumber = await business.giveOrders(manufacturer1).length;
            ...
            await business.orderInProgress(1, {from:deployer}).should.be.rejected; // need to check if this is correct

           await business.orderInProgress(1, {from:manufacturer2}).should.be.rejected; // need to check if this is correct
           ...
           const event = result.logs[0].args;
           assert.equal(event.status,'inProgress', 'Order name is correct');
        ...
        it ('Orderjjjjjjjj status to be updated as inProgress only by the assigned manufacturer', async ()=>{
            //  orderNumber = await business.giveOrders(manufacturer1).length;
  
            await business.orderCompleted(1, {from:company}).should.be.rejected; // need to check if this is correct
  
            await business.orderCompleted(1, {from:manufacturer2}).should.be.rejected; // need to check if this is correct
