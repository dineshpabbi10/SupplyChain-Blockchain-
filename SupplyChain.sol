...
contract supplyChain {
    ...
    function orderCompleted(uint256 _orderId) public payable {
        require(
            msg.sender != companyAddress,
            "Company can not set the order to be in progress, only manufacturer can do that"
        );
    ...
    function createProduct(string memory _productName, uint256 _makingPrice)
        public
        Owner
    {
        inventory memory inventory1 = companyInventory[msg
            .sender][_productName];
        uint256 price = 0;
    ...
    function checkStock(
        ...
        Order memory order1 = giveOrders[msg.sender][_orderId];
        inventory memory inventory1 = companyInventory[companyAddress][order1
            .name];
