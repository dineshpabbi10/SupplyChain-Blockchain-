...
contract supplyChain {
    ...
    modifier Owner {
        require(
            ...
            "Only Admin Company can call this function"
        );
        _;
    }
    ...
    function giveOrderToManufacturer(string memory _Componentname, uint256 number, address _manufacturerCompany) public payable Owner {
        require(number > 0);
    ...
    function distributorRequirement(string memory _productName,uint256 quantity) public payable {
    ...
    function checkStock(uint256 _orderId,address _distributor,address payable _transporter ) public payable Owner {
        Order memory order1 = giveOrders[msg.sender][_orderId];
    ...
    function transportStocktoDistributor(uint256 _transportationcost,address payable transporteraddress) public payable {
        transporteraddress.transfer(_transportationcost);
    }

    //1 . The retailer orders for the required product in this function
    function retailerRequirement(string memory _productName,uint256 quantity,address _distributor) public payable {
        require(msg.sender != companyAddress);
    ...
    function transportStocktoRetailer( uint256 _transportationcost, address payable transporteraddress ) public payable {
        transporteraddress.transfer(_transportationcost);
    }
    ...
    function seeDetails(string memory _productName) public {
        ...
        for (uint256 i = 0; i < productComponentList.length; i++) {
            productDetailsMapping[_productName].push(string(abi.encodePacked((productComponentList[i].componentName), (" is manufactured by "), (productComponentList[i].manufacturer))));
        }
    }
