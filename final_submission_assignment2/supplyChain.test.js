...
contract(SupplyChain,([deployer, manufacturer, retailer, transporter, customer])=>{
    let supplyChain;
    before(async () =>{
    ...
    describe('Deployment', async()=>{
        ...
        it('The deployed smart contract has the correct name', async()=>{
            const name = await supplyChain.company_name();
            assert.equal(name, 'MIHIR MANUFACTURERS')
        })
    })
    ...
    describe('Adding and sellig item', async()=>{
        ...
        before(async ()=>{
            result = await ethbay.giveOrderToManufacturer('thread', web3.utils.toWei('1', 'Ether'),{from: deployer})
            const event = result.logs[0].args;
            assert.equal(event.order1.name, "thread",'item name is correct');
        })
